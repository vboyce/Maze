...
def run_french(sentences):
    '''wrapper for using the frwac model
    Arguments:
    ...
    import french
    end_result = {}
    model, device = french.load_model() #set up
    dictionary, ntokens = french.load_dict()
    for i, _ in enumerate(sentences): #process all the sentences
        bad = french.do_sentence_set(sentences[i], model, device, dictionary, ntokens)
...
end_result=run_french(sentences)
